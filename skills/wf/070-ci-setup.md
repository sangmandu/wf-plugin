...
- [ ] ```bash
  bash ${CLAUDE_PLUGIN_ROOT}/skills/wf/lib/discover-ci.sh
  ```
  - Exit 0 → done. Config is up to date.
  - Non-zero → step failure. Inspect stderr.
- [ ] Verify: `python3 ${CLAUDE_PLUGIN_ROOT}/skills/wf/lib/config.py repos.$(jq -r .data.project_name .workflow/state.json).ci_checks`
